...
minimum_to_draw = paras['minima_to_draw'].split()
...
for dir in state_listdir:
   ...
   if paras['emphasize_state_start']:
      if dir >= paras['emphasize_state_start'] and dir <= paras['emphasize_state_end']:
         emphasize.append(rs)
   if paras['specified_state'] is not None:
      if str(dir) in paras['specified_state']:
         specified_min.append(rs)

...
for dir in state_listdir:
   ...
   Au_coord[dir], surface_Au[dir] = get_coord(atoms)


process_names=['step-number', 'reactant-id', 'process-id', 'product-id','step-time','total-time','barrier', 'rate','energy']
dynamics_table = pd.read_table('dynamics.txt', delimiter = r'\s+', skiprows=[0,1], names=process_names)
for i in range(len(dynamics_table['product-id'])):
   state_id = dynamics_table['product-id'].iloc[i]
   print i, state_e[state_id], Au_coord[state_id], surface_Au[state_id]
